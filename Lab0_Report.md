...
我最近与另一名同学合作搭建 Linguistic Online Judge 网站，使用 GitHub 托管代码。我们按功能分工，在各自的分支上开发，再通过 Pull Request 合并到 `main`。例如，我曾分别通过功能分支提交题目目录 API 和题目浏览界面的修改，对应仓库中的 PR #29 和 PR #31。协作中也遇到过代码覆盖和修改冲突，这让我认识到，多人开发不仅需要完成各自的功能，还需要及时同步代码，并在合并时仔细检查修改内容。
...
我阅读了《语义化版本 2.0.0》。语义化版本使用 `主版本号.次版本号.修订号` 的格式：当出现不兼容的 API 修改时增加主版本号；当加入向下兼容的新功能时增加次版本号；当进行向下兼容的问题修正时增加修订号。规范还定义了先行版本和构建信息的表示方式。
